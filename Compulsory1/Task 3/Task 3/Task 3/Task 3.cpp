...
float calculateFunctionValue(float x, float y)
{
    return x * x + y * y;
}

void writeFunctionDataToFile(const char* fileName, int numPoints, float min, float max, float steps)
{
    ...
    for(float i= 0; i< numPoints ; i++)
    {
	    for(float j = 0; j< numPoints ; j++)
	    {
            float x = i * steps + min;
            float y = j * steps + min;
		    float z = calculateFunctionValue(x, y);

            x = x / 2; // scale down the x value
            y = y / 2; // scale down the y value
            z = z / 2; // scale down the z value

		    outputFile << std::fixed << std::setprecision(3);
		    outputFile << x << ", " << y << ", " << z << ", 1.0, 0.0, 0.0" << std::endl;
	    }
    }
...
void drawFunction(const char* fileName) {
    ...
    // create window
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "3D-Function", NULL, NULL);
    if (window == NULL) {
...
int main()
{
    ...
    float a = 0.0f; //min
    float b = 1.0f; //max
    float h = (b - a) / numPoints; // step size

   writeFunctionDataToFile(fileName, numPoints, a, b, h);

   drawFunction(fileName);
